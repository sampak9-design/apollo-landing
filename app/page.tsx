...
export default function Home() {
  return (
    <main className="relative min-h-screen w-full overflow-hidden bg-apollo-dark">
      ...
      {/* Hero */}
      <section className="relative z-30 flex min-h-[calc(100vh-100px)] flex-col items-center justify-center px-6 text-center">
        {/* Tag */}
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-apollo-cyan/30 bg-apollo-cyan/5 px-4 py-1.5 backdrop-blur-sm">
          <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-apollo-cyan" />
          <span className="text-xs font-semibold uppercase tracking-[0.2em] text-apollo-cyan">
            Inteligência Artificial · 2026
          </span>
        </div>

        {/* Headline */}
        <h1 className="mb-6 max-w-4xl font-display text-4xl font-black uppercase leading-[1.05] tracking-tight sm:text-6xl md:text-7xl lg:text-8xl">
        ...
        {/* Subheadline */}
        <p className="mb-12 max-w-2xl text-lg leading-relaxed text-white/70 sm:text-xl">
        ...
        {/* Trust line */}
        <p className="mt-8 text-sm uppercase tracking-widest text-white/40">
          + de <span className="text-apollo-cyan">15.000</span> membros ativos
        </p>

        {/* Stats grid */}
        <div className="mt-16 grid w-full max-w-3xl grid-cols-2 gap-4 border-t border-apollo-cyan/10 pt-10 sm:grid-cols-3">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-center">
      <span className="font-display text-2xl font-bold text-apollo-cyan neon-text sm:text-3xl">
        {value}
      </span>
